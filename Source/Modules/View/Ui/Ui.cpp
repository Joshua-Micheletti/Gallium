...
UI::UI(Renderer* m_renderer, EventHandler* m_eventHandler) {
	...
	this->m_showMetricsWindow = false;
	this->m_showUserGuide = false;
	this->m_showAboutWindow = false;
	this->m_showDemoWindow = false;
	this->m_showFPS = true;
	this->m_pauseFlag = false;
	this->m_showLeftColumn = true;
	this->m_showRightColumn = true;
	this->m_showBottomRow = true;
...
void UI::drawLeftColumn() {
	...
	if (firstDraw) {
		ImGui::SetWindowSize(ImVec2(250, window.height() - this->m_menuBarSize.y));
		firstDraw = false;
	}
	
	if (updateResolution) {
		ImGui::SetWindowSize(ImVec2(ImGui::GetWindowWidth(), window.height() - this->m_menuBarSize.y));
	}

	this->m_leftColumnSize = ImGui::GetWindowSize();

	// this->m_renderer->setHighlightedEntity(selected);
	
	
	ImGui::SetWindowPos(ImVec2(0, this->m_menuBarSize.y));
...
void UI::drawRightColumn() {
	...
	if (ImGui::CollapsingHeader("Post Processing Effect")) {
		...
		this->m_renderer->setPostProcessingEffect(selectedEffect);

		ImGui::Separator();

		if (selectedEffect == 2) {
			static float m_color[3] = { 1.0, 1.0, 1.0 };
			ImGui::ColorEdit3("Color", m_color);

			this->m_renderer->setFilterColor(m_color[0], m_color[1], m_color[2]);
		}
		...
		if (selectedEffect == 9) {
			...
			//this->m_renderer->setKernelConvolutionMode(selectedKernel);
			if (selectedKernel == 2) {
				this->m_renderer->Kernels->setEdgeDetectionKernel();
				ImGui::SameLine();
				this->HelpMarker("|-1|-1|-1|\n|-1| 8|-1|\n|-1|-1|-1|");
			}
			else if (selectedKernel == 3) {
				this->m_renderer->Kernels->setSharpeningKernel();
				ImGui::SameLine();
				this->HelpMarker("| 0|-1| 0|\n|-1| 5|-1|\n| 0|-1| 0|");
			}
			else if (selectedKernel == 4) {
				this->m_renderer->Kernels->setEmbossKernel();
				ImGui::SameLine();
				this->HelpMarker("|-2|-1| 0|\n|-1| 1| 1|\n| 0| 1| 2|");
			}
			else if (selectedKernel == 5) {
				this->m_renderer->Kernels->setHorizontalKernel();
				ImGui::SameLine();
				this->HelpMarker("|-1|-1|-1|\n| 2| 2| 2|\n|-1|-1|-1|");
			}
			else if (selectedKernel == 6) {
				this->m_renderer->Kernels->setVerticalKernel();
				ImGui::SameLine();
				this->HelpMarker("|-1| 2|-1|\n|-1| 2|-1|\n|-1| 2|-1|");
			}
			else if (selectedKernel == 7) {
				this->m_renderer->Kernels->setDiagonalSXKernel();
				ImGui::SameLine();
				this->HelpMarker("| 2|-1|-1|\n|-1| 2|-1|\n|-1|-1| 2|");
			}
			else if (selectedKernel == 8) {
				this->m_renderer->Kernels->setDiagonalDXKernel();
				ImGui::SameLine();
			...
			if (selectedKernel == 0 || selectedKernel == 1) {
				...
				if (selectedKernel == 0) {
					...
					this->m_renderer->Kernels->setGaussianKernel(blurStrength, kernelSize);
				}
				else {
					this->m_renderer->Kernels->setBoxBlurKernel(kernelSize);
				}
			}
			...
			if (selectedKernel == 9) {
				...
				this->m_renderer->Kernels->setCustomKernel(a, b, c, d, e, f, g, h, i);
			}
		}
	}
	...
	ImGui::SetWindowPos(ImVec2(window.width() - ImGui::GetWindowWidth(), this->m_menuBarSize.y));
	this->m_rightColumnSize = ImGui::GetWindowSize();
...
void UI::drawBottomRow() {
	...
	if (firstDraw) {
		...
		if (this->m_showLeftColumn) {
			offset += this->m_leftColumnSize.x;
		}

		if (this->m_showRightColumn) {
			offset += this->m_rightColumnSize.x;
		}
	...
	if (this->m_showLeftColumn) {
		ImGui::SetWindowPos(ImVec2(this->m_leftColumnSize.x, window.height() - size.y));
	}
	else {
...
void UI::drawFPSWindow() {
	...
	if (this->m_showFPS == true) {
		ImGui::Begin("Frame Rate", &this->m_showFPS, FPSwindowFlags);
		...
		if (this->m_showRightColumn) {
			ImGui::SetWindowPos(ImVec2(window.width() - ImGui::GetWindowSize().x - this->m_rightColumnSize.x, 0 + this->m_menuBarSize.y));
		}
		else {
			ImGui::SetWindowPos(ImVec2(window.width() - ImGui::GetWindowSize().x, 0 + this->m_menuBarSize.y));
		}
...
void UI::drawMenuBar() {
	if (ImGui::BeginMainMenuBar()) {
		...
		if (ImGui::BeginMenu("View")) {
			ImGui::MenuItem("FPS", NULL, &this->m_showFPS);
			ImGui::MenuItem("Left Column", NULL, &this->m_showLeftColumn);
			ImGui::MenuItem("Right Column", NULL, &this->m_showRightColumn);
			ImGui::MenuItem("Bottom Row", NULL, &this->m_showBottomRow);

			ImGui::EndMenu();
		}

		if (ImGui::BeginMenu("ImGui")) {
			if (ImGui::MenuItem("Metrics Window", NULL, &this->m_showMetricsWindow));

			if (ImGui::MenuItem("User Guide", NULL, &this->m_showUserGuide));

			if (ImGui::MenuItem("About Window", NULL, &this->m_showAboutWindow));

			if (ImGui::MenuItem("Demo", NULL, &this->m_showDemoWindow));

			ImGui::EndMenu();
		}

		this->m_menuBarSize = ImGui::GetWindowSize();

		ImGui::EndMainMenuBar();
	}

	if (this->m_showMetricsWindow == true) {
		ImGui::ShowMetricsWindow(&this->m_showMetricsWindow);
	}

	if (this->m_showUserGuide == true) {
		ImGui::ShowUserGuide();
	}

	if (this->m_showAboutWindow == true) {
		ImGui::ShowAboutWindow(&this->m_showAboutWindow);
	}

	if (this->m_showDemoWindow == true) {
		ImGui::ShowDemoWindow(&this->m_showAboutWindow);
	}

	if (this->m_pauseFlag == true) {
		this->m_eventHandler->setUpdateFlag(false);
	}
	else {
		this->m_eventHandler->setUpdateFlag(true);
	}
}
...
void UI::drawImGui() {
	...
	if (this->m_showLeftColumn == true) {
		this->drawLeftColumn();
	}

	if (this->m_showRightColumn == true) {
		this->drawRightColumn();
	}
	
	if (this->m_showBottomRow == true) {
		this->drawBottomRow();
	}
...
void UI::drawInfo() {
	...
	this->m_fpsTime = glfwGetTime() - tmpTime;
	tmpTime = glfwGetTime();
