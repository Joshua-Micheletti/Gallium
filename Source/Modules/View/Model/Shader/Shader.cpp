...
std::vector<std::string> Shader::layoutBuffer() {
	return(this->m_layoutBuffer);
}

void Shader::loadShader(std::string vertex, std::string fragment) {
	this->m_vertexSource = vertex;
	this->m_fragmentSource = fragment;

	this->m_id = compileShader(vertex.c_str(), fragment.c_str());
	...
	while (fscanf(vertexShader, "%s", buffer) != EOF) {
		...
		if (readingUniform) {
			if (readingUniform == 2) {
				...
				tmpUniform.id = glGetUniformLocation(this->m_id, tmpUniform.name);
				this->m_uniformBuffer.push_back(tmpUniform);
				tmpUniform.name = (char*)calloc(sizeof(char), 255);
				tmpUniform.type = (char*)calloc(sizeof(char), 255);
			}
			readingUniform--;
		...
		if (readingLayouts) {
			if (readingLayouts == 1) {
				...
				this->m_layoutBuffer.push_back(&tmpLayout[0]);
...
void Shader::printFull() {
	printf("Vertex: %s\nFragment: %s\n", this->m_vertexSource.c_str(), this->m_fragmentSource.c_str());
	printf("Uniforms:\n");
	for (int i = 0; i < this->m_uniformBuffer.size(); i++) {
		printf("\t%d: %s %s\n", this->m_uniformBuffer[i].id, this->m_uniformBuffer[i].type, this->m_uniformBuffer[i].name);
	}

	printf("Layouts:\n");
	for (int i = 0; i < this->m_layoutBuffer.size(); i++) {
		printf("\t%s\n", this->m_layoutBuffer[i].c_str());
	}
}